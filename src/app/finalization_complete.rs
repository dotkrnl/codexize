...
impl App {
    /// Cross-stage finalization dispatcher.
    ///
    /// Owns only the routing concerns common to every stage:
    /// pending-termination intent, `Operator Killed`/`user_forced_retry`
    /// suppression, the final-validation blocked branch, and the auto-retry
    /// fall-through. Per-stage success handling lives co-located with the
    /// stage's launch in `src/app_runtime/stages/<name>.rs`.
