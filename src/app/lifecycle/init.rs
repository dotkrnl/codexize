...
impl App {
    ...
    pub(crate) fn new_with_startup_origin_config_without_model_refresh(
        ...
        let supervisor = SessionSupervisor::new(state, startup_origin, config);
        Self::attach_to_supervisor(supervisor, false)
    }
    ...
    fn new_with_startup_origin_config_and_model_refresh(
        ...
        let supervisor = SessionSupervisor::new(state, startup_origin, config);
        Self::attach_to_supervisor(supervisor, eager_model_refresh)
    }

    pub fn attach_to_supervisor(
        ...
        if supervisor.state.builder.task_titles.is_empty() {
            ...
            let tasks_path = if config.paths.sessions_root.is_explicit() {
                paths_view.sessions_root.join(&supervisor.state.session_id)
            } else {
                session_state::codexize_root()
                    .join("sessions")
                    .join(&supervisor.state.session_id)
            };
        ...
        let nodes = build_tree(&supervisor.state);
        ...
        let retention = app.supervisor.config.memory_view().journal_retention_months;
        match crate::data::memory::prune_journal_entries(&memory_root, retention) {
            Ok(0) => {}
            Ok(n) => {
                let _ = app.supervisor.state.log_event(format!(
                    "journal_pruned: removed={n} retention_months={retention}"
                ));
            }
            Err(err) => {
                let _ = app.supervisor.state.log_event(format!("journal_prune_failed: {err}"));
            }
        }
        if eager_model_refresh {
            ...
            // section is expired.
            let loaded = cache::load(&app.supervisor.config.paths_view().cache_root);
        ...
        if let Ok(run_id) = session_state::resume_running_runs(&mut app.supervisor.state) {
            app.supervisor.current_run_id = run_id;
            app.supervisor.run_launched = run_id.is_some();
            if let Some(rid) = run_id {
                if let Some(run) = app.supervisor.state.agent_runs.iter().find(|r| r.id == rid).cloned() {
                    app.supervisor.live_summary_path = Some(app.live_summary_path_for(&run));
                    app.prime_yolo_exit_tracking(&run);
                }
                app.read_live_summary_pipeline();
            }
            app.supervisor.messages = SessionState::load_messages(&app.supervisor.state.session_id).unwrap_or_default();
        ...
        if app.supervisor.state.current_phase == Phase::GitGuardPending {
            if app.supervisor.state.pending_guard_decision.is_none() {
                ...
                let _ = app.supervisor.state.save();
            }
        } else if app.supervisor.state.pending_guard_decision.is_some() {
            // Stale: pending decision with no matching phase — clear it.
            let _ = app.supervisor.state.log_event(
                "warning: clearing stale pending_guard_decision (phase mismatch on resume)"
                    .to_string(),
            );
            session_state::clear_pending_guard_decision(&mut app.supervisor.state);
            let _ = app.supervisor.state.save();
        }
    ...
    pub fn detach_supervisor(self) -> SessionSupervisor {
        self.supervisor
    }
}
#[cfg(test)]
