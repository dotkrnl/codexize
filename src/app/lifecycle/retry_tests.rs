...
//! `state.current_stage` lands on the right variant, and the special-case
...
fn fresh_state(stage: Stage) -> SessionState {
...
fn go_back_from_plan_rewinds_to_spec_and_deletes_plan_artifact() {
    with_temp_root(|| {
        ...
        // op; the lane-gate accepts isolated-session stages.
        app.go_back();
