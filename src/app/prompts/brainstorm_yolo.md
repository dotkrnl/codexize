...
Workflow:
  ...
  2. Resolve every ambiguity yourself per the trust preamble — pick the
     narrowest reasonable reading and note the choice in `## Assumptions`.
...
Spec structure reminder — include this stub exactly, even if empty:
    ...
    ## Out of scope
    <!-- Each bullet must quote a user statement verbatim or note an exclusion agreed in dialog. Never invent exclusions; pick the narrowest reading and record it in `## Assumptions` if uncertain. -->
