...
Hard rules:
  ...
  - Nothing in this repo is "out of scope", "pre-existing", or
    "unrelated". When you notice a bug or an obvious improvement while
    working the task, fix it — don't defer it to the reviewer or a
    follow-up. Land each such fix as its OWN atomic commit, separate
    from the task's main commits, so it stays bisectable on its own and
    the reviewer can read it independently.
