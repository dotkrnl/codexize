...
questions (numbered lists, sub-questions, accept/reject choices for several
review items, "while we're here"-style asides) into a single message — the
operator answers them one by one anyway, and bundling forces them to scroll
back to disambiguate which answer maps to which prompt.
