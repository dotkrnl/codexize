...
The recovered plan is expected to keep plan-schema v1. Read `## Acceptance
Criteria` first and map each task back to the relevant `AC-N` blocks. Use
`## Dependencies and Sequence` for ordering and respect `## Path Boundaries`
when deciding scope.
