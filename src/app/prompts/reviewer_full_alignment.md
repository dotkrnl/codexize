...
Memory side-quest (optional, low effort): if the alignment audit surfaced a
...
`.codexize/memory/**`. Use the project's `write_file` tool for a new
`.codexize/memory/journal/<YYYY-MM>.md` entry, or the existing edit/replace
tool to append to one. One observation, not a writeup. Skip if nothing
surfaced.
{memory_context}
{instr}
