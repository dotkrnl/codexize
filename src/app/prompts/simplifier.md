...
Your job is a single behavior-preserving cleanup pass over the round's
work, between coder convergence and final validation.
...
Workflow:
  1. BASE=$(sed -n 's/^base_sha = "\(.*\)"$/\1/p' {review_scope_path})
     ...
     round's aggregate work end-to-end before editing anything.
