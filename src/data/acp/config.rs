...
impl AcpConfig {
    ...
    pub fn resolve(&self, request: &AcpLaunchRequest) -> AcpResult<AcpResolvedLaunch> {
        ...
        let join = |paths: &[PathBuf]| paths.iter().map(|p| p.display().to_string()).collect::<Vec<_>>().join("\n");
...
pub fn claude_cli_is_available() -> bool { program_is_executable(CLAUDE_CLI) }
pub fn should_offer_claude_acp_install() -> bool { program_is_executable(CLAUDE_CLI) && !claude_acp_is_available() }
pub fn codex_acp_is_available() -> bool { program_is_executable(CODEX_ACP_CLI) }
pub fn codex_cli_is_available() -> bool { program_is_executable(CODEX_CLI) }
pub fn should_offer_codex_acp_install() -> bool { program_is_executable(CODEX_CLI) && !codex_acp_is_available() }
...
fn absolutize_policy(policy: &AcpLaunchPolicy) -> AcpResult<AcpLaunchPolicy> {
    Ok(AcpLaunchPolicy {
        allowed_write_paths: policy.allowed_write_paths.iter().map(|p| absolutize(p)).collect::<AcpResult<Vec<_>>>()?,
