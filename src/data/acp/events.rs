...
pub fn translate_update(update: ClientUpdate, interactive: bool) -> Option<AcpRuntimeEvent> {
    ...
    Some(match update {
        ...
        ClientUpdate::ToolCallActivity { tool_call_id, kind } => AcpRuntimeEvent::ToolCallActivity { tool_call_id, kind },
        ClientUpdate::SessionInfoUpdate { title } => return title.map(|title|
            AcpRuntimeEvent::Lifecycle(AcpLifecycleEvent::SessionTitleUpdated { title })),
        ClientUpdate::PromptTurnFinished => AcpRuntimeEvent::Completion(AcpCompletionEvent::PromptTurnFinished),
        ClientUpdate::PromptTurnFailed { message } => AcpRuntimeEvent::Completion(AcpCompletionEvent::PromptTurnFailed { message }),
