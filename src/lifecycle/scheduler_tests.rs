...
fn ctx<'a>(stage: Stage, prior: &'a [RunHistoryEntry], pending: &'a [u32]) -> StageCtx<'a> {
    StageCtx {
        session_id: "s",
...
// --- StageRegistry: next_stage_for_stage ----------------------------------
...
fn empty_registry_returns_none_for_every_stage() {
    ...
    for stage in [
        ...
        assert_eq!(reg.next_stage_for_stage(stage, &ctx(stage, &[], &[])), None);
    }
}
...
fn default_registry_returns_brainstorm_at_idea() {
    ...
    assert_eq!(
        reg.next_stage_for_stage(Stage::Idea, &ctx(Stage::Idea, &[], &[])),
...
fn default_registry_returns_none_at_idea_when_brainstorm_done() {
    ...
    // Stage Idea after Brainstorm succeeded: the registry honestly reports
    // None because no stage at Stage::Idea has more work. The FSM would
    // have already advanced the stage before reaching this state; the
    // test pins down the registry-level contract.
    assert_eq!(
        reg.next_stage_for_stage(Stage::Idea, &ctx(Stage::Idea, &prior, &[])),
        None
    );
}

#[test]
fn default_registry_plan_stage_walks_candidates_in_order() {
    let reg = default_registry();
    // Nothing done yet → Planning wins.
    assert_eq!(
        reg.next_stage_for_stage(Stage::Plan, &ctx(Stage::Plan, &[], &[])),
        Some(StageId::Planning)
    );
    // After Planning Done → PlanReview wins.
    let after_planning = [done_run(StageId::Planning, None, 1)];
    assert_eq!(
        reg.next_stage_for_stage(Stage::Plan, &ctx(Stage::Plan, &after_planning, &[])),
        Some(StageId::PlanReview)
    );
    // PlanReview is multi-round-per-stage (advances the round counter on
...
fn default_registry_implementation_returns_coder_when_tasks_pending() {
    ...
    assert_eq!(
        reg.next_stage_for_stage(
            Stage::Implementation(1),
            &ctx(Stage::Implementation(1), &[], &pending)
        ),
        Some(StageId::Coder)
    );
}
...
fn default_registry_implementation_skips_recovery_when_gate_closed() {
    ...
    // Coder Done, no recovery gate → no stage to schedule at this stage.
    assert_eq!(
        reg.next_stage_for_stage(
            Stage::Implementation(1),
            &ctx(Stage::Implementation(1), &prior, &pending)
        ),
        None
    );
}
...
fn default_registry_implementation_selects_recovery_when_gate_open() {
    ...
    let pending = [1u32];
    let prior = [done_run(StageId::Coder, Some(1), 1)];
    let mut c = ctx(Stage::Implementation(1), &prior, &pending);
    c.recovery_active = true;
    assert_eq!(
        reg.next_stage_for_stage(Stage::Implementation(1), &c),
        Some(StageId::Recovery)
    );
}

#[test]
fn default_registry_review_stage_simplification_gated() {
    let reg = default_registry();
    let pending = [1u32];
    // Reviewer not done → Reviewer first.
    assert_eq!(
        reg.next_stage_for_stage(Stage::Review(1), &ctx(Stage::Review(1), &[], &pending)),
    ...
    assert_eq!(
        reg.next_stage_for_stage(
            Stage::Review(1),
            &ctx(Stage::Review(1), &after_reviewer, &pending)
        ),
        None
    );
    // Reviewer done + simplification requested → Simplification.
    let mut c = ctx(Stage::Review(1), &after_reviewer, &pending);
    c.simplification_requested = true;
    assert_eq!(
        reg.next_stage_for_stage(Stage::Review(1), &c),
...
fn default_registry_finalization_dreaming_gated() {
    ...
    assert_eq!(
        reg.next_stage_for_stage(Stage::Finalization, &ctx(Stage::Finalization, &[], &[])),
    ...
    assert_eq!(
        reg.next_stage_for_stage(
            Stage::Finalization,
            &ctx(Stage::Finalization, &after_fv, &[])
        ),
        None
    );
    // With dreaming_accepted → Dreaming.
    let mut c = ctx(Stage::Finalization, &after_fv, &[]);
    c.dreaming_accepted = true;
    assert_eq!(
        reg.next_stage_for_stage(Stage::Finalization, &c),
        Some(StageId::Dreaming)
    );
}

#[test]
fn default_registry_terminal_stages_return_none() {
    let reg = default_registry();
...
fn stages_after_plan_includes_every_implementation_review_and_finalization_stage() {
    let reg = default_registry();
    let got = reg.stages_after(Stage::Plan);
    // Set membership: every stage whose canonical stage_when_running > Plan.
    ...
    // Ordering invariant: stages must be non-increasing through the result.
    // Use each stage's `stage_when_running()` for the comparison; equal
    // stages (e.g. Reviewer / Simplification) may appear in either order
    // since the contract doesn't pin the tie-break.
    let stages: Vec<Stage> = got
        .iter()
        .map(|id| reg.get(*id).expect("registered").stage_when_running())
        .collect();
...
fn stages_after_finalization_is_empty() {
    let reg = default_registry();
    assert!(reg.stages_after(Stage::Finalization).is_empty());
}

#[test]
fn stages_after_cancelled_is_empty() {
    // Cancelled is incomparable with every other stage → no hits.
    let reg = default_registry();
    assert!(reg.stages_after(Stage::Cancelled).is_empty());
}
...
#[test]
fn plan_blocks_terminal_stage_before_paused() {
    let sched = Scheduler::new(default_registry());
...
#[test]
fn plan_blocks_when_paused_matches_current_stage() {
    let sched = Scheduler::new(default_registry());
    let agent = AgentState::Idle;
    let pending = PendingDecisions::default();
    let mut input = baseline_input(&agent, Stage::Plan, &pending, ctx(Stage::Plan, &[], &[]));
    input.paused_at_stage = Some(Stage::Plan);
    assert_eq!(sched.plan(input), TickOutcome::Blocked(BlockReason::Paused));
}

#[test]
fn plan_ignores_pause_at_other_stage() {
    let sched = Scheduler::new(default_registry());
    let agent = AgentState::Idle;
    let pending = PendingDecisions::default();
    let mut input = baseline_input(&agent, Stage::Plan, &pending, ctx(Stage::Plan, &[], &[]));
    input.paused_at_stage = Some(Stage::Spec); // not current
...
fn plan_blocks_on_project_lane() {
    ...
    assert_eq!(
        sched.plan(input),
        TickOutcome::Blocked(BlockReason::ProjectLane)
    );
}
...
fn plan_returns_idle_when_no_candidate_has_pending_work() {
    ...
    let agent = AgentState::Idle;
    let pending = PendingDecisions::default();
    // Brainstorm already done → next_stage_for_stage returns None.
    let prior = [done_run(StageId::Brainstorm, None, 1)];
    assert_eq!(
...
fn plan_block_precedence_agent_busy_beats_terminal() {
    ...
    let pending = PendingDecisions::default();
    // Even on Done stage, AgentBusy wins.
    let outcome = sched.plan(baseline_input(
        &agent,
        Stage::Done,
        &pending,
