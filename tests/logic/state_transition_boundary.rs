...
fn production_source_files(dir: &Path, files: &mut Vec<std::path::PathBuf>) {
    for entry in fs::read_dir(dir).expect("read source dir") {
        ...
        if path.is_dir() {
            // Layer-owned `tests/` directories hold private suites declared
            // from production modules; they are not production source.
            if name.is_some_and(|n| n == "tests" || n.starts_with("tests_")) {
                continue;
            }
